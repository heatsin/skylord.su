...
/* ==========================================================================
   ...
   ========================================================================== */
const DATA = {
  // Привилегии (месяц/квартал/навсегда). Каждая следующая включает всё из предыдущей.
  privilegeTiers: [
    { id: 'palach',    icon: '🩸', name: 'Палач',     month: 29,  quarter: 69,   forever: 149,
      prefix: '[Палач] ВашНик',
      kit: '/kit Палач — стартовый набор.',
      perks: { homes: 2, regions: 2, auction: 6, delay: 7 } },
    { id: 'strazh',    icon: '🛡', name: 'Страж',     month: 59,  quarter: 149,  forever: 299,
      prefix: '[Страж] ВашНик',
      kit: 'Всё из Палача + /kit Страж.',
      perks: { homes: 4, regions: 4, auction: 10, delay: 6 } },
    { id: 'berserk',   icon: '⚔', name: 'Берсерк',   month: 99,  quarter: 249,  forever: 499,
      prefix: '[Берсерк] ВашНик',
      kit: 'Всё из Стража + /kit Берсерк, /feed.',
      perks: { homes: 6, regions: 6, auction: 15, delay: 5 } },
    { id: 'hunter',    icon: '🏹', name: 'Охотник',   month: 149, quarter: 349,  forever: 699,
      prefix: '[Охотник] ВашНик',
      kit: 'Всё из Берсерка + /kit Охотник, /smithingtable.',
      perks: { homes: 8, regions: 8, auction: 18, delay: 4 } },
    { id: 'paladin',   icon: '⚜', name: 'Паладин',   month: 249, quarter: 599,  forever: 1199,
      prefix: '[Паладин] ВашНик',
      kit: 'Всё из Охотника + /kit Паладин, /craft.',
      perks: { homes: 10, regions: 10, auction: 21, delay: 3 } },
    { id: 'titan',     icon: '⚡', name: 'Титан',     month: 399, quarter: 999,  forever: 1999,
      prefix: '[Титан] ВашНик',
      kit: 'Всё из Паладина + /kit Титан, /repair.',
      perks: { homes: 12, regions: 12, auction: 24, delay: 2 } },
    { id: 'lord',      icon: '👑', name: 'Властелин', month: 699, quarter: 1699, forever: 3299,
      prefix: '[Властелин] ВашНик',
      kit: 'Всё из Титана + /kit Властелин, /anvil.',
      perks: { homes: 15, regions: 15, auction: 28, delay: 1 },
      extra: ['⭐ Приоритетный вход на сервер'] },
    { id: 'supreme',   icon: '🌌', name: 'Верховный', month: 999, quarter: 2499, forever: 4999, featured: true,
      prefix: '[Верховный] ВашНик',
      kit: 'Всё из Властелина + /kit Верховный, /ec, /workbench, /stonecutter.',
      perks: { homes: 20, regions: 20, auction: 35, delay: 0 },
      extra: ['⭐ Максимальный приоритет входа', '👑 Уникальный префикс и статус на сервере'] },
  ],
...
function renderPrivileges() {
  const grid = $('#privilege-grid');
  if (!grid) return;

  grid.innerHTML = DATA.privilegeTiers.map((tier, i) => `
    <article class="shop-card${tier.featured ? ' is-featured' : ''}" style="--tier-glow:${i % 2 === 0 ? 'var(--blue)' : 'var(--violet)'}">
      ${tier.featured ? '<span class="shop-card-badge">Топ уровень</span>' : ''}
      <span class="shop-card-icon shop-card-icon--emoji" aria-hidden="true">${tier.icon}</span>
      <h3 class="shop-card-name">${tier.name}</h3>
      <p class="shop-card-prefix"><code>${tier.prefix}</code></p>
      <p class="shop-card-desc">${tier.kit}</p>

      <ul class="shop-card-perks">
        <li>🏠 Точек дома: <b>${tier.perks.homes}</b></li>
        <li>🛡 Регионов: <b>${tier.perks.regions}</b></li>
        <li>📦 Слотов на аукционе: <b>${tier.perks.auction}</b></li>
        <li>⏳ Задержка телепорта: <b>${tier.perks.delay > 0 ? tier.perks.delay + ' сек' : 'без задержки'}</b></li>
        ${(tier.extra || []).map((e) => `<li>${e}</li>`).join('')}
      </ul>
      ...
      <button type="button" class="btn btn--primary shop-card-cta" data-buy-item="${tier.id}">Купить</button>
    </article>
  `).join('');

  // Переключение срока действия — просто визуальный выбор (оплата ещё не подключена).
  $$('#privilege-grid .shop-card').forEach((card) => {
  ...
  $$('#privilege-grid [data-buy-item]').forEach((btn) => {
